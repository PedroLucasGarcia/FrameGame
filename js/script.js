...
function loadProgress() {
    let currentLevel = parseInt(localStorage.getItem('currentLevel')) || 1; // Nível atual salvo ou 1 por padrão
    const totalLevels = 50; // Número total de níveis (modifique conforme necessário)

    // Seleciona todos os botões de fase
    const buttons = document.querySelectorAll('.buttonLevel');

    buttons.forEach(button => {
        const level = parseInt(button.id.replace('level', ''));

        // Define o estilo de acordo com o progresso
        if (level < currentLevel) {
            button.classList.add('completed'); // Fase concluída
        } else if (level === currentLevel) {
            button.classList.add('current'); // Fase atual
        } else {
            button.classList.add('locked'); // Fase bloqueada
            button.parentElement.href = "javascript:void(0);"; // Desativa o link para fases bloqueadas
            button.disabled = true; // Desativa o link para fases bloqueadas
        }
    });
}
...
// Chame esta função quando o jogador concluir um nível
